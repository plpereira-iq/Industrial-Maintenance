...
const ListaMantenimientosPendientes = () => {
  ...
  const handleInformar = (id, eqName, eqCode, mainDate, mainResponsible) => {
    axios.delete(`http://localhost:8000/api/maintenances/${id}`)
      .then(response => {
        navigate(`/informe-mantenimiento/${eqName}/${eqCode}/${mainDate}/${mainResponsible}`);
      })
      .catch(error => {
  ...
  const handleReprogramar = (id, eqName, eqCode) => {
    navigate(`/reprogramar-mantenimiento/${id}`);
    console.log(id);
  };
        ...
        <div className="list-group">
          {mantenimientos.map((mantenimiento, index) => (
            <div key={index} className="list-group-item list-group-item-action flex-column align-items-start">
              ...
              <button className="btn btn-primary mt-2" onClick={() => handleInformar(mantenimiento._id, mantenimiento.eqName, mantenimiento.eqCode,  mantenimiento.mainDate, mantenimiento.mainResponsible)}>Informar</button>
              <button className="btn btn-secondary mt-2 ml-2" onClick={() => handleReprogramar(mantenimiento._id, mantenimiento.eqName, mantenimiento.eqCode)}>Reprogramar Mantenimiento</button>
            </div>
          ))}
        </div>
      </div>
